<!-- <script context="module">
	export async function load({ fetch, url }) {
		const jsonUrl = url.pathname;
		// const jsonUrl = '/fixing-favicons';
		const res = await fetch(jsonUrl);
		console.log(jsonUrl, res, url);
		if (res.ok) {
			return {
				props: {
					post: await res.json()
...
<main class="container py-8 min-h-screen">
	<h1
		class="text-slate-700 dark:text-white font-bold tracking-tight leading-tight  "
		...
		<li class="text-slate-500 text-lg text-justify mt-4  max-w-xl leading-relaxed ">
			...
			<div class="flex flex-row">
				{#each post.tags as tag}
					<span class="border border-red-400 rounded-2xl p-3">
						{tag}
					</span>
				{/each}
			</div>
