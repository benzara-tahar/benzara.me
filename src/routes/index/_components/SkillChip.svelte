...
>
	{#if skill.image}
		<img
			class="w-10 h-10 p-1 transition bg-cover rounded-full group-hover:grayscale-0 max-w-none dark:bg-gray-900 bg-slate-300 group-hover:bg-primary-500"
			alt={skill.name}
			src={skill.image}
		/>
	{/if}
	<span class="hidden p-2 text-sm text-gray-700 dark:text-gray-200 sm:inline-block">
		{skill.name}
	</span>
</span>
